...
do
    for i in `seq 1 32`;
    do
        $ROSETTA_BUILD_DIR/$program results/$protein/decoys/$i proteins/$protein/pdb 500 rmsd >> results/$protein/best_ca_rmsd
        $ROSETTA_BUILD_DIR/$program results/$protein/decoys/$i proteins/$protein/pdb 500 gdt >> results/$protein/best_ca_gdt
